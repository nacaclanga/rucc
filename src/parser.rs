use crate::lexer::{Keyword, Lexer, Pos, Symbol, Token, TokenKind};
use crate::node::{ASTKind, Bits, AST};
use crate::node;
use crate::types::{Sign, StorageClass, Type};
...
macro_rules! expect_symbol_error {
    ($slf:expr, $sym:expr, $msg:expr) => {{
        if !r#try!($slf.lexer.skip_symbol($sym)) {
            let peek = $slf.lexer.peek();
            $slf.show_error_token(&r#try!(peek), $msg);
        }
    }}
}
...
impl<'a> Parser<'a> {
    ...
    fn read_func_def(&mut self) -> ParseR<AST> {
        ...
        let (ret_ty, _, _qualifiers) = r#try!(self.read_type_spec());
        let (functy, name, param_names) = r#try!(self.read_declarator(ret_ty));
        ...
        let body = r#try!(self.read_func_body(&functy));
    ...
    fn read_compound_stmt(&mut self) -> ParseR<AST> {
        ...
        loop {
            if r#try!(self.lexer.skip_symbol(Symbol::ClosingBrace).or_else(|eof| {
            ...
            let peek_tok = r#try!(self.lexer.peek());
            if self.is_type(&peek_tok) {
                // variable declaration
                r#try!(self.read_decl(&mut stmts));
            } else {
    ...
    fn read_stmt(&mut self) -> ParseR<AST> {
        ...
        if matches!(tok.kind, TokenKind::Identifier(_))
            && r#try!(self.lexer.peek_symbol_token_is(Symbol::Colon))
        {
    ...
    fn read_if_stmt(&mut self) -> ParseR<AST> {
        ...
        let cond = r#try!(self.read_expr());
        expect_symbol_error!(self, Symbol::ClosingParen, "expected ')'");
        let then_stmt = Box::new(r#try!(self.read_stmt()));
        let else_stmt = if r#try!(self.lexer.skip_keyword(Keyword::Else)) {
            Box::new(r#try!(self.read_stmt()))
        } else {
    ...
    fn read_for_stmt(&mut self) -> ParseR<AST> {
        expect_symbol_error!(self, Symbol::OpeningParen, "expected '('");
        let init = r#try!(self.read_opt_decl_or_stmt());
        // TODO: make read_expr return Option<AST>.
        //       when cur tok is ';', returns None.
        let cond = r#try!(self.read_opt_expr());
        expect_symbol_error!(self, Symbol::Semicolon, "expected ';'");
        let step = if r#try!(self.lexer.peek_symbol_token_is(Symbol::ClosingParen)) {
            AST::new(ASTKind::Compound(Vec::new()), self.lexer.get_cur_pos())
        } else {
            r#try!(self.read_opt_expr())
        };
        expect_symbol_error!(self, Symbol::ClosingParen, "expected ')'");
        let body = r#try!(self.read_stmt());
        Ok(AST::new(
            ASTKind::For(
                Box::new(init),
                Box::new(cond),
    ...
    fn read_do_while_stmt(&mut self) -> ParseR<AST> {
        let body = r#try!(self.read_stmt());
        if !r#try!(self.lexer.skip_keyword(Keyword::While)) {
            let peek = self.lexer.peek();
            self.show_error_token(&r#try!(peek), "expected 'while'");
        }
        expect_symbol_error!(self, Symbol::OpeningParen, "expected '('");
        let cond = r#try!(self.read_expr());
        expect_symbol_error!(self, Symbol::ClosingParen, "expected ')'");
    ...
    fn read_switch_stmt(&mut self) -> ParseR<AST> {
        ...
        let cond = r#try!(self.read_expr());
        expect_symbol_error!(self, Symbol::ClosingParen, "expected ')'");
        let body = Box::new(r#try!(self.read_stmt()));
        Ok(AST::new(
    ...
    fn read_goto_stmt(&mut self) -> ParseR<AST> {
        ...
        let label_name = ident_val!(r#try!(self.lexer.get()));
        expect_symbol_error!(self, Symbol::Semicolon, "expected ';'");
    ...
    fn read_return_stmt(&mut self) -> ParseR<AST> {
        let pos = self.lexer.get_cur_pos();
        if r#try!(self.lexer.skip_symbol(Symbol::Semicolon)) {
            Ok(AST::new(ASTKind::Return(None), pos))
        } else {
            let retval = Some(Box::new(r#try!(self.read_expr())));
            expect_symbol_error!(self, Symbol::Semicolon, "expected ';'");
    ...
    fn is_function_def(&mut self) -> ParseR<bool> {
        ...
        loop {
            let mut tok = r#try!(self.lexer.get());
            buf.push(tok.clone());
            ...
            if tok.kind == TokenKind::Symbol(Symbol::OpeningParen) {
                r#try!(self.skip_parens(&tok, &mut buf));
                continue;
            }
            ...
            if r#try!(self.lexer.peek()).kind != TokenKind::Symbol(Symbol::OpeningParen) {
                continue;
            }

            let opening_paren = r#try!(self.lexer.get());
            buf.push(opening_paren.clone());
            r#try!(self.skip_parens(&opening_paren, &mut buf));

            tok = r#try!(self.lexer.peek());
    ...
    fn skip_parens(&mut self, opening_paren: &Token, buf: &mut Vec<Token>) -> ParseR<()> {
        loop {
            let tok = r#try!(self.lexer.get().or_else(|_| {
            ...
            match tok.kind {
                TokenKind::Symbol(Symbol::OpeningParen) => r#try!(self.skip_parens(&tok, buf)),
    ...
    fn read_decl_init(&mut self, ty: &mut Type) -> ParseR<AST> {
        ...
        if r#try!(self.lexer.peek_symbol_token_is(Symbol::OpeningBrace)) {
    ...
    fn read_initializer_elem(&mut self, ty: &mut Type) -> ParseR<AST> {
        ...
        } else if r#try!(self.lexer.peek_symbol_token_is(Symbol::OpeningBrace)) {
    ...
    fn read_array_initializer(&mut self, ty: &mut Type) -> ParseR<AST> {
        let has_brace = r#try!(self.lexer.skip_symbol(Symbol::OpeningBrace));
        ...
        if let &mut Type::Array(ref elem_ty, ref mut len) = ty {
            ...
            loop {
                ...
                self.lexer.unget(tok);
                let elem = r#try!(self.read_initializer_elem(&mut elem_ty));
                elems.push(elem);
                r#try!(self.lexer.skip_symbol(Symbol::Comma));
            }
            if is_flexible {
    ...
    fn read_struct_initializer(&mut self, ty: &mut Type) -> ParseR<AST> {
        ...
        loop {
            ...
            self.lexer.unget(tok);
            let elem = r#try!(self.read_initializer_elem(&mut field_type.next().unwrap().clone()));
            elems.push(elem);
            r#try!(self.lexer.skip_symbol(Symbol::Comma));
        }
        Ok(AST::new(
    ...
    fn skip_type_qualifiers(&mut self) -> ParseR<()> {
        while r#try!(self.lexer.skip_keyword(Keyword::Const))
            || r#try!(self.lexer.skip_keyword(Keyword::Volatile))
            || r#try!(self.lexer.skip_keyword(Keyword::Restrict))
        {}
        Ok(())
    }
    fn read_decl(&mut self, ast: &mut Vec<AST>) -> ParseR<()> {
        let (basety, sclass, qualifiers) = r#try!(self.read_type_spec());
        ...
        loop {
            let (mut ty, name, _) = r#try!(self.read_declarator(basety.clone())); // XXX

            if (qualifiers.q_constexpr || qualifiers.q_const)
                && r#try!(self.lexer.skip_symbol(Symbol::Assign))
            {
                let init = r#try!(self.read_decl_init(&mut ty));
                ...
                let init = if r#try!(self.lexer.skip_symbol(Symbol::Assign)) {
                    Some(Box::new(r#try!(self.read_decl_init(&mut ty))))
                } else {
                    None
                };
                self.env.add(
            ...
            if !r#try!(self.lexer.skip_symbol(Symbol::Comma)) {
                let peek = r#try!(self.lexer.get());
    ...
    fn read_declarator(&mut self, basety: Type) -> ParseR<(Type, String, Option<Vec<String>>)> {
        if r#try!(self.lexer.skip_symbol(Symbol::OpeningParen)) {
            let peek_tok = r#try!(self.lexer.peek());
            if self.is_type(&peek_tok) {
                let (ty, params) = r#try!(self.read_declarator_func(basety));
                return Ok((ty, "".to_string(), params));
            }

            // TODO: HUH? MAKES NO SENSE!!
            let mut buf: Vec<Token> = Vec::new();
            while !r#try!(self.lexer.skip_symbol(Symbol::ClosingParen)) {
                buf.push(r#try!(self.lexer.get()));
            }
            let t = r#try!(self.read_declarator_tail(basety));
            self.lexer.unget_all(&buf);
            return self.read_declarator(t.0);
        }

        if r#try!(self.lexer.skip_symbol(Symbol::Asterisk)) {
            r#try!(self.skip_type_qualifiers());
        ...
        if let &TokenKind::Identifier(ref name) = &tok.kind {
            let (ty, params) = r#try!(self.read_declarator_tail(basety));
    ...
    fn read_declarator_tail(&mut self, basety: Type) -> ParseR<(Type, Option<Vec<String>>)> {
        if r#try!(self.lexer.skip_symbol(Symbol::OpeningBoxBracket)) {
            return Ok((r#try!(self.read_declarator_array(basety)), None));
        }
        if r#try!(self.lexer.skip_symbol(Symbol::OpeningParen)) {
    ...
    fn read_declarator_array(&mut self, basety: Type) -> ParseR<Type> {
        let len: i32;
        if r#try!(self.lexer.skip_symbol(Symbol::ClosingBoxBracket)) {
            len = -1;
        } else {
            len = match r#try!(self.read_expr()).eval_constexpr() {
                Ok(len) => len as i32,
                Err(Error::Something) => {
                    let peek = r#try!(self.lexer.peek());
        ...
        let ty = r#try!(self.read_declarator_tail(basety)).0;
        Ok(Type::Array(Box::new(ty), len))
    }
    fn read_declarator_func(&mut self, retty: Type) -> ParseR<(Type, Option<Vec<String>>)> {
        if r#try!(self.lexer.peek_keyword_token_is(Keyword::Void))
            && r#try!(self.lexer.next_symbol_token_is(Symbol::ClosingParen))
        {
            r#try!(self.lexer.expect_skip_keyword(Keyword::Void));
            r#try!(self.lexer.expect_skip_symbol(Symbol::ClosingParen));
            return Ok((Type::Func(Box::new(retty), Vec::new(), false), None));
        }
        if r#try!(self.lexer.skip_symbol(Symbol::ClosingParen)) {
            return Ok((Type::Func(Box::new(retty), Vec::new(), false), None));
        }

        let (paramtypes, paramnames, vararg) = r#try!(self.read_declarator_params());
        Ok((
    ...
    fn read_declarator_params(&mut self) -> ParseR<(Vec<Type>, Vec<String>, bool)> {
        ...
        loop {
            if r#try!(self.lexer.skip_symbol(Symbol::Vararg)) {
                if paramtypes.len() == 0 {
                    let peek = self.lexer.peek();
            ...
            let (ty, name) = r#try!(self.read_func_param());
            ...
            if !r#try!(self.lexer.skip_symbol(Symbol::Comma)) {
                let peek = self.lexer.peek();
                self.show_error_token(&r#try!(peek), "expected ','");
    ...
    fn read_func_param(&mut self) -> ParseR<(Type, String)> {
        let basety = r#try!(self.read_type_spec()).0;
        let (ty, name, _) = r#try!(self.read_declarator(basety));
        match ty {
        ...
        loop {
            ...
            if kind.is_none() {
                if let &TokenKind::Identifier(ref maybe_userty_name) = &tok.kind {
                    let maybe_userty = r#try!(self.get_typedef(maybe_userty_name));
            ...
            if let TokenKind::Keyword(keyw) = tok.kind {
                match &keyw {
                    ...
                    &Keyword::Void => {
                        if kind.is_some() {
                            let peek = self.lexer.peek();
                            self.show_error_token(&r#try!(peek), "type mismatch");
                        }
                    ...
                    &Keyword::Struct => userty = Some(r#try!(self.read_struct_def())),
                    &Keyword::Union => userty = Some(r#try!(self.read_union_def())),
                    &Keyword::Enum => userty = Some(r#try!(self.read_enum_def())),
                    _ => {}
                }
            } else {
                self.lexer.unget(tok);
    ...
    fn read_rectype_tag(&mut self) -> ParseR<Option<String>> {
        let maybe_tag = r#try!(self.lexer.get());
    ...
    fn read_rectype_def(&mut self, is_struct: bool) -> ParseR<Type> {
        let tag = {
            let opt_tag = r#try!(self.read_rectype_tag());
            if opt_tag.is_some() {
                opt_tag.unwrap()
            } else {
        ...
        let fields = r#try!(self.read_rectype_fields());
    ...
    fn read_rectype_fields(&mut self) -> ParseR<Vec<AST>> {
        if !r#try!(self.lexer.skip_symbol(Symbol::OpeningBrace)) {
            return Ok(Vec::new());
        }
        ...
        loop {
            ...
            let (basety, _, _) = r#try!(self.read_type_spec());
            loop {
                let (ty, name, _) = r#try!(self.read_declarator(basety.clone()));
                if r#try!(self.lexer.skip_symbol(Symbol::Colon)) {
                    // TODO: for now, designated bitwidth ignore
                    r#try!(self.read_expr());
                }
                decls.push(AST::new(
    ...
    fn read_enum_def(&mut self) -> ParseR<Type> {
        ...
        if exist_tag {
            match self.tags.get(tag.as_str()) {
                ...
                _ => {
                    let peek = self.lexer.peek();
                    self.show_error_token(&r#try!(peek), "undefined enum");
                    return Err(Error::Something);
                }
            }
        }
        ...
        if !r#try!(self.lexer.skip_symbol(Symbol::OpeningBrace)) {
            if !exist_tag || !self.tags.contains(tag.as_str()) {
                let peek = self.lexer.peek();
                self.show_error_token(&r#try!(peek), "do not redefine enum");
                return Err(Error::Something);
            }
        ...
        let mut val = 0;
        loop {
            if r#try!(self.lexer.skip_symbol(Symbol::ClosingBrace)) {
                break;
            }
            let name = ident_val!(r#try!(self.lexer.get()));
            if r#try!(self.lexer.skip_symbol(Symbol::Assign)) {
                val = match r#try!(self.read_assign()).eval_constexpr() {
                    Ok(val) => val,
                    Err(Error::Something) => {
                        let peek = r#try!(self.lexer.peek());
    ...
    pub fn read_opt_expr(&mut self) -> ParseR<AST> {
        if r#try!(self.lexer.peek()).kind == TokenKind::Symbol(Symbol::Semicolon) {
            Ok(AST::new(
    ...
    fn read_comma(&mut self) -> ParseR<AST> {
        let mut lhs = r#try!(self.read_assign());
        while r#try!(self.lexer.skip_symbol(Symbol::Comma)) {
            let rhs = r#try!(self.read_assign());
            lhs = AST::new(
    ...
    fn read_assign(&mut self) -> ParseR<AST> {
        let mut lhs = r#try!(self.read_logor());
        if r#try!(self.lexer.skip_symbol(Symbol::Question)) {
        ...
        macro_rules! f { ($op:ident) => (
            lhs = assign!(
                ...
                AST::new(
                    ASTKind::BinaryOp(
                        Box::new(lhs),
                        Box::new(r#try!(self.read_assign())),
                        node::CBinOps::$op,
                    ),
                    self.lexer.get_cur_pos(),
                ),
        ...
        loop {
            ...
            match tok.kind {
                TokenKind::Symbol(Symbol::Assign) => {
                    lhs = assign!(lhs, r#try!(self.read_assign()), self.lexer.get_cur_pos());
                }
    ...
    fn read_ternary(&mut self, cond: AST) -> ParseR<AST> {
        let mut then_expr = r#try!(self.read_expr());
        expect_symbol_error!(self, Symbol::Colon, "expected ':'");
        let mut else_expr = r#try!(self.read_assign());
        let then_ty = r#try!(self.get_expr_returning_ty(&then_expr));
        let else_ty = r#try!(self.get_expr_returning_ty(&else_expr));
    ...
    fn read_logor(&mut self) -> ParseR<AST> {
        let mut lhs = r#try!(self.read_logand());
        while r#try!(self.lexer.skip_symbol(Symbol::LOr)) {
            let rhs = r#try!(self.read_logand());
            lhs = AST::new(
    ...
    fn read_logand(&mut self) -> ParseR<AST> {
        let mut lhs = r#try!(self.read_or());
        while r#try!(self.lexer.skip_symbol(Symbol::LAnd)) {
            let rhs = r#try!(self.read_or());
            lhs = AST::new(
    ...
    fn read_or(&mut self) -> ParseR<AST> {
        let mut lhs = r#try!(self.read_xor());
        while r#try!(self.lexer.skip_symbol(Symbol::Or)) {
            let rhs = r#try!(self.read_xor());
            lhs = AST::new(
    ...
    fn read_xor(&mut self) -> ParseR<AST> {
        let mut lhs = r#try!(self.read_and());
        while r#try!(self.lexer.skip_symbol(Symbol::Xor)) {
            let rhs = r#try!(self.read_and());
            lhs = AST::new(
    ...
    fn read_and(&mut self) -> ParseR<AST> {
        let mut lhs = r#try!(self.read_eq_ne());
        while r#try!(self.lexer.skip_symbol(Symbol::Ampersand)) {
            let rhs = r#try!(self.read_eq_ne());
            lhs = AST::new(
    ...
    fn read_eq_ne(&mut self) -> ParseR<AST> {
        let mut lhs = r#try!(self.read_relation());
        loop {
            if r#try!(self.lexer.skip_symbol(Symbol::Eq)) {
                let rhs = r#try!(self.read_relation());
                lhs = AST::new(
                    ASTKind::BinaryOp(Box::new(lhs), Box::new(rhs), node::CBinOps::Eq),
                    self.lexer.get_cur_pos(),
                );
            } else if r#try!(self.lexer.skip_symbol(Symbol::Ne)) {
                let rhs = r#try!(self.read_relation());
                lhs = AST::new(
    ...
    fn read_relation(&mut self) -> ParseR<AST> {
        let mut lhs = r#try!(self.read_shl_shr());
        loop {
            if r#try!(self.lexer.skip_symbol(Symbol::Lt)) {
                let rhs = r#try!(self.read_shl_shr());
                lhs = AST::new(
                    ASTKind::BinaryOp(Box::new(lhs), Box::new(rhs), node::CBinOps::Lt),
                    self.lexer.get_cur_pos(),
                );
            } else if r#try!(self.lexer.skip_symbol(Symbol::Le)) {
                let rhs = r#try!(self.read_shl_shr());
                lhs = AST::new(
                    ASTKind::BinaryOp(Box::new(lhs), Box::new(rhs), node::CBinOps::Le),
                    self.lexer.get_cur_pos(),
                );
            } else if r#try!(self.lexer.skip_symbol(Symbol::Gt)) {
                let rhs = r#try!(self.read_shl_shr());
                lhs = AST::new(
                    ASTKind::BinaryOp(Box::new(lhs), Box::new(rhs), node::CBinOps::Gt),
                    self.lexer.get_cur_pos(),
                );
            } else if r#try!(self.lexer.skip_symbol(Symbol::Ge)) {
                let rhs = r#try!(self.read_shl_shr());
                lhs = AST::new(
    ...
    fn read_shl_shr(&mut self) -> ParseR<AST> {
        let mut lhs = r#try!(self.read_add_sub());
        loop {
            if r#try!(self.lexer.skip_symbol(Symbol::Shl)) {
                let rhs = r#try!(self.read_add_sub());
                lhs = AST::new(
                    ASTKind::BinaryOp(Box::new(lhs), Box::new(rhs), node::CBinOps::Shl),
                    self.lexer.get_cur_pos(),
                );
            } else if r#try!(self.lexer.skip_symbol(Symbol::Shr)) {
                let rhs = r#try!(self.read_add_sub());
                lhs = AST::new(
    ...
    fn read_add_sub(&mut self) -> ParseR<AST> {
        let mut lhs = r#try!(self.read_mul_div_rem());
        loop {
            if r#try!(self.lexer.skip_symbol(Symbol::Add)) {
                let rhs = r#try!(self.read_mul_div_rem());
                lhs = AST::new(
                    ASTKind::BinaryOp(Box::new(lhs), Box::new(rhs), node::CBinOps::Add),
                    self.lexer.get_cur_pos(),
                );
            } else if r#try!(self.lexer.skip_symbol(Symbol::Sub)) {
                let rhs = r#try!(self.read_mul_div_rem());
                lhs = AST::new(
    ...
    fn read_mul_div_rem(&mut self) -> ParseR<AST> {
        let mut lhs = r#try!(self.read_cast());
        loop {
            if r#try!(self.lexer.skip_symbol(Symbol::Asterisk)) {
                let rhs = r#try!(self.read_cast());
                lhs = AST::new(
                    ASTKind::BinaryOp(Box::new(lhs), Box::new(rhs), node::CBinOps::Mul),
                    self.lexer.get_cur_pos(),
                );
            } else if r#try!(self.lexer.skip_symbol(Symbol::Div)) {
                let rhs = r#try!(self.read_cast());
                lhs = AST::new(
                    ASTKind::BinaryOp(Box::new(lhs), Box::new(rhs), node::CBinOps::Div),
                    self.lexer.get_cur_pos(),
                );
            } else if r#try!(self.lexer.skip_symbol(Symbol::Mod)) {
                let rhs = r#try!(self.read_cast());
                lhs = AST::new(
    ...
    fn read_cast(&mut self) -> ParseR<AST> {
        ...
        if tok.kind == TokenKind::Symbol(Symbol::OpeningParen) && self.is_type(&peek) {
            let basety = r#try!(self.read_type_spec()).0;
            let ty = r#try!(self.read_declarator(basety)).0;
            expect_symbol_error!(self, Symbol::ClosingParen, "expected ')'");
            return Ok(AST::new(
                ASTKind::TypeCast(Box::new(r#try!(self.read_cast())), ty),
                self.lexer.get_cur_pos(),
            ));
        } else {
            self.lexer.unget(tok);
    ...
    fn read_unary(&mut self) -> ParseR<AST> {
        ...
        match tok.kind {
            TokenKind::Symbol(Symbol::Not) => {
                return Ok(AST::new(
                    ASTKind::UnaryOp(Box::new(r#try!(self.read_cast())), node::CUnaryOps::LNot),
                    self.lexer.get_cur_pos(),
                ))
            }
            TokenKind::Symbol(Symbol::BitwiseNot) => {
                return Ok(AST::new(
                    ASTKind::UnaryOp(Box::new(r#try!(self.read_cast())), node::CUnaryOps::BNot),
                    self.lexer.get_cur_pos(),
                ))
            }
            ...
            TokenKind::Symbol(Symbol::Sub) => {
                return Ok(AST::new(
                    ASTKind::UnaryOp(Box::new(r#try!(self.read_cast())), node::CUnaryOps::Minus),
                    self.lexer.get_cur_pos(),
                ))
            }
            TokenKind::Symbol(Symbol::Inc) => {
                let pos = self.lexer.get_cur_pos();
                let var = r#try!(self.read_cast());
                return Ok(AST::new(
                    ASTKind::BinaryOp(
                        Box::new(var.clone()),
                        Box::new(AST::new(
            ...
            TokenKind::Symbol(Symbol::Asterisk) => {
                return Ok(AST::new(
                    ASTKind::UnaryOp(Box::new(r#try!(self.read_cast())), node::CUnaryOps::Deref),
                    self.lexer.get_cur_pos(),
                ))
            }
            TokenKind::Symbol(Symbol::Ampersand) => {
                return Ok(AST::new(
                    ASTKind::UnaryOp(Box::new(r#try!(self.read_cast())), node::CUnaryOps::Addr),
                    self.lexer.get_cur_pos(),
                ))
            }
    ...
    fn read_sizeof(&mut self) -> ParseR<AST> {
        ...
        if matches!(tok.kind, TokenKind::Symbol(Symbol::OpeningParen)) && self.is_type(&peek) {
            let (basety, _, _) = r#try!(self.read_type_spec());
            let (ty, _, _) = r#try!(self.read_declarator(basety));
            r#try!(self.lexer.skip_symbol(Symbol::ClosingParen));
            return Ok(AST::new(
        ...
        self.lexer.unget(tok);
        let expr = r#try!(self.read_unary());
        Ok(AST::new(
            ASTKind::Int(r#try!(self.calc_sizeof(&expr)) as i64, Bits::Bits32),
            self.lexer.get_cur_pos(),
        ))
    }
    fn read_postfix(&mut self) -> ParseR<AST> {
        let mut ast = r#try!(self.read_primary());
        loop {
            if r#try!(self.lexer.skip_symbol(Symbol::OpeningParen)) {
                ast = r#try!(self.read_func_call(ast));
                continue;
            }
            if r#try!(self.lexer.skip_symbol(Symbol::OpeningBoxBracket)) {
                ast = AST::new(
                    ASTKind::Load(Box::new(r#try!(self.read_index(ast)))),
                    self.lexer.get_cur_pos(),
                );
                continue;
            }
            if r#try!(self.lexer.skip_symbol(Symbol::Point)) {
                ast = AST::new(
                    ASTKind::Load(Box::new(r#try!(self.read_field(ast)))),
                    self.lexer.get_cur_pos(),
                );
                continue;
            }
            if r#try!(self.lexer.skip_symbol(Symbol::Arrow)) {
                let pos = self.lexer.get_cur_pos();
                let field = r#try!(self.read_field(AST::new(
            ...
            if r#try!(self.lexer.skip_symbol(Symbol::Inc)) {
                return Ok(AST::new(
                    ASTKind::UnaryOp(Box::new(ast), node::CUnaryOps::Inc),
                    self.lexer.get_cur_pos(),
                ));
            }
            if r#try!(self.lexer.skip_symbol(Symbol::Dec)) {
                return Ok(AST::new(
    ...
    fn read_field(&mut self, ast: AST) -> ParseR<AST> {
        let field = r#try!(self.lexer.get());
        if !matches!(field.kind, TokenKind::Identifier(_)) {
            let peek = self.lexer.peek();
            self.show_error_token(&r#try!(peek), "expected field name");
            return Err(Error::Something);
        }
    ...
    fn get_binary_expr_ty(&mut self, lhs: &AST, rhs: &AST, op: &node::CBinOps) -> ParseR<Type> {
        ...
        let lhs_ty = cast(r#try!(self.get_expr_returning_ty(lhs)));
        let rhs_ty = cast(r#try!(self.get_expr_returning_ty(rhs)));
    ...
    fn get_expr_returning_ty(&mut self, ast: &AST) -> ParseR<Type> {
        let size = match ast.kind {
            ...
            ASTKind::Load(ref v) => {
                (*r#try!(self.get_expr_returning_ty(&*v)).get_elem_ty().unwrap()).clone()
            }
            ...
            | ASTKind::UnaryOp(ref expr, node::CUnaryOps::BNot) => {
                r#try!(self.get_expr_returning_ty(&*expr))
            }
            ASTKind::UnaryOp(ref expr, node::CUnaryOps::Deref) => (*r#try!(
                self.get_expr_returning_ty(&*expr)
            ).get_elem_ty()
                .unwrap())
                .clone(),
            ASTKind::UnaryOp(ref expr, node::CUnaryOps::Addr) => {
                Type::Ptr(Box::new(r#try!(self.get_expr_returning_ty(&*expr))))
            }
            ASTKind::StructRef(ref expr, ref name) => {
                let ty = r#try!(self.get_expr_returning_ty(expr));
            ...
            ASTKind::BinaryOp(ref lhs, ref rhs, ref op) => {
                r#try!(self.get_binary_expr_ty(&*lhs, &*rhs, &*op))
            }
            ASTKind::TernaryOp(_, ref then, _) => r#try!(self.get_expr_returning_ty(&*then)),
            ASTKind::FuncCall(ref func, _) => {
                let func_ty = r#try!(self.get_expr_returning_ty(func));
    ...
    fn calc_sizeof(&mut self, ast: &AST) -> ParseR<usize> {
        let ty = r#try!(self.get_expr_returning_ty(ast));
        Ok(ty.calc_size())
    }
